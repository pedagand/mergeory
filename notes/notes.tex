...
The elisions can be seen as code moved from a location in $d$ into another location in $i$, but the actual content is temporarily forgotten inside what we call a meta-variable. In this paper, we will denote these elisions by $\alpha, \beta, \gamma,$ and so on.
...
\begin{itemize}
  ...
  \item If an elided node do not match an elision with the same hash in the other tree, replace it by the original sub-tree.
  ...
  \item Optionally, replace each hash by a meta-variable index for readability and simplicity.
\end{itemize}
...
In the merging algorithm we use the interpretation of meta-variables as multi-source multi-target move, and try to push changes toward the destination of the code movement.
