...
%
To keep track of the changes, and automate these fusions, developers use dedicated tools named \textit{version control systems}. The most used ones are Git and Mercurial, but others exist with different approaches like Subversion (centralized, older) or Darcs and Pijul (category-based). \todo{refs}
...
the final code\footnote{Modern coding techniques involve continuous
  integration testing to mitigate that, but tests cases are not
  perfect, so we want to expose another approach.}.
...
code, a system like Git accepts to merge them automatically.
Unfortunately, even if two modifications are physically
...
In this work, we want to give a semantically sound notion of commits merge which
...
For semantic guarantees, we offer a practical, expressive, and original notion of unambiguous merge. This part is specific to a particular programming language, but we demonstrate how it works for languages that feature both functionnal and imperative features, to make it easily adaptable.
...
\subsection{Problems with line-based approach}
Merging changes while preserving the original intent of the developers is non trivial. The usual method used in version control systems takes a line-based approach. In a merge operation the input files consist of a common base and at least two modified versions. The usual algorithm compare the base against each modification to create a line to line alignment minimizing changes. Then it tries to merge these changes. If the same line in the base is modified twice, it declares a conflict, else it applies the single change. This method works on any kind of textual data, but is mainly used for code. However, it fails to capture any semantic, and this can cause undesired conflicts or worse create bugs.
...
On the example above, we show two problems with the line-based approach. First, the refactoring in blue does not compose with value changes in orange. This creates the conflict shown above. Secondly, the fusion of both codes creates a runtime bug at line 15: the implicit assumption that y is non null is broken by orange at line 11, but this is silently accepted. We say that when $c$ is true, $x / y$ is an ambiguous expression because its result comes only from the fusion itself. Moreover, we can notice that the absence of conflict on lines 10 to 14 is only due to the code formatting style, if the same if-expression was written on a single line, there would be a conflict there. Globally, the example shows that by not knowing anything about how a program is executed, textual merge creates conflicts both too often to accept refactorings and not enough to avoid introducing bugs.
...
\paragraph{Two phase approach} Directly merging programs changes semantically is hard to do and a result would be hard to retranscribe into source code. So we adopt here a two phase approach. We first syntactically merge both commits, and then we run a semantic analysis on it to deduce which program states are ambiguous. A program state is semantically ambiguous if it comes from a modification whose invariants may have been broken by another modification. Initially, as the fusion algorithm is not aware of the invariants, any value that comes from none of the commits in particular but only their combination will generate an ambiguity.
These ambiguity points, should then be manually reviewed, and resolved.

\paragraph{Syntactic merge}
Similarly to what line-based merge do, we first want to compute individual differences between the common base and each of the commits. However here, differences are not based on lines of code but on the syntax tree itself. This means that differences are syntax trees of an extended language. In addition to normal code, they can have insertions, deletions, local replacement, unchanged holes (depicted as $\id$ below), or multi-source multi-target code moves (depicted as Greek letters) to abstract away sub-trees that are irrelevant for the commit and ease fusion later.

To keep track of which commit introduced which modification we use colors. Each commit is associated with a tint. In the example, left commit has blue tint, and right commit has orange tint. We can see in their diff that they taint the code they introduced and the modification they made.
...
After having generated individual syntactic differences, we can merge them and remove holes by reusing the original program, producing the code above in the middle. This process is designed such that it never makes any arbitrary choices, and can sometimes generate conflicts. However, it tries whenever possible to push the changes following moved code to ease refactoring fusions (see lines 15 and 1).

At this point, we capture refactorings based on code moves transparently, make syntactic conflicts happen less often and enforce some code structure in the changes. However, we lose the ability to commit files that do not parse (or we need to fallback to another merge method in that case), and we still do not provide any guarantee about ambiguities.

\paragraph{Semantic check}
The key idea of the semantic check is to interpret a difference as an interleaving of the merged program to the original base. On this interleaving, we derive an analysis that checks the absence of ambiguous program state. Ambiguity can be linked to the color black (i.e. without any tint), because it means that nobody is aware of the value. The necessity of doing this analysis on an interleaving and not directly on the resulting program comes from the fact that deleted instructions must be taken into account. \gb{Too technical? Should I remove the mention of interleaving here?}

In our example the color black appears for instance at line 11 when $c$ is true, because variable $x$ gets a blue color at line 4, and variable $y$ gets an orange color at line 7, therefore value at line 11 is not known by any commit. It also appears at line 16 and 17 as orange is not aware that its code has been moved, and blue could theoretically have reordered instructions.

To finish this example, we need a user to be able to efficiently resolve the conflicts. Here saying that on lines 2 to 5 and 14 to 18, blue has done only semantic preserving changes will replace the blue color by white in each of these lines. Then, an ambiguity line 11 will still remain, it could be resolved by overriding the expression in the merged commit by a new one provided by the user like for instance \lstinline$if y == 0 { 0 } else { x / y }$.
Therefore, given this information, the two commits can be safely merged with the guarantee that no unintended behavior\footnote{Side channel attack vectors might still be silently introduced unless we also model them into the analysis.} will be silently introduced by the fusion itself.
...
\centering\begin{tikzpicture}
    \node[varnode] (del_plus) {$+$};
    \node[mvnode, colorA] (del_alpha) at ($(del_plus)+(-0.75,-1.25)$) {$\alpha$};
    \node[mvnode, colorB] (del_beta) at ($(del_plus)+(0.75,-1.25)$) {$\beta$};
    \draw (del_plus) -- (del_alpha);
    \draw (del_plus) -- (del_beta);

    \node[varnode] (ins_minus) at ($(del_plus)+(4,0)$) {$-$};
    \node[mvnode, colorA] (ins_alpha) at ($(ins_minus)+(-0.75,-1.25)$) {$\alpha$};
    \node[mvnode, colorB] (ins_beta) at ($(ins_minus)+(0.75,-1.25)$) {$\beta$};
...
\paragraph{Merging the spine} Trying to merge together two nodes in the syntax tree is easy, we simply need to check if they use the same root constructor, recursively merge if it is the case, and fail otherwise. However, this approach shows its limits in presence of sequences (eg. sequences of functions or sequences of statements). If we consider that the sequence constructor contains its size, we are forced to fully split the list if only one element has been inserted or removed. Else, if we encode the sequence with Cons/Nil constructors, elements will always be inserted/removed at the end, generating  sub-optimal differences. Therefore, we use list edit scripts instead as the output of a merge operation on two sequences. We call the merged part of the difference tree its spine.
...
\paragraph{Syntax of syntactic differences} We denote by $S(\overrightarrow{t}, \overrightarrow{s})$ a constructor from the target language syntax, with an arbitrary but fixed arity of trees ($t$) and sequences ($s$). For instance one of the possible $S$ could be the language construction \lstinline[mathescape]|if $t_{cond}$ { $\overrightarrow{s_{true}}$ } else { $\overrightarrow{s_{false}}$ }| with one expression sub-tree and two nested sequences of instructions.
...
To compute a syntactic difference, we take syntax trees of both the original and the modified program and we run the following algorithm:
\begin{itemize}
  \item Compute a hash for each node of each syntax tree.
  \item Take the subset of hashes occurring both in original and modified syntax tree.
  ...
  \item Merge both syntax trees into a common spine where possible, else keep changes as two separate elided trees. Always try to align sequences using recursively a dynamic algorithm minimizing the number of nodes of the resulting tree (same algorithm as Git, but with elements instead of lines).
...
Note that for having a production ready tool, the current parser is not adapted, as it systematically forgets the comments inside source code. But this is left for future work as a pure engineering problem.
...
The white color having all the tints, it represent a consensus of all the commits. On the other side, black represent the fact that no single commit can claim responsibility for a given value. This makes the white color the good choice for everything already present in the original file, and black a color that we want to track and avoid because it represent an ambiguity.

\newbox\boxdelinner
\sbox\boxdelinner{\setul{-.75ex}{}$\mathul{x}^c$}
\newbox\boxinsinner
\sbox\boxinsinner{\setul{.1ex}{}$\mathul{x}^c$}
All modifications carry a color. If the exact same modification is done twice, the result has the union of tints as its color as any of the two commits know the modification.
...
\begin{align*}
{\color{red}c} &= {\color{red}S(\overrightarrow{c}, \overrightarrow{c})} \typsep {\color{red}\alpha}\\
...
Note that unchanged parts are never colored, and that syntax nodes are not colored in the spine but are in the changed sub-trees. This reflects the fact that in the spine and inside holes, code comes from the original source code, while elsewhere it is introduced by the commit.

\subsection{Merging differences}
Now that we have difference trees for each individual concurrent change, we can try to merge them in a way that propagates awareness colors for each piece of code.
...
If we want to merge more than two differences, we can reapply the same algorithm on already merged trees, thus accumulating modifications but also conflicts. Before moving on to the semantic analysis, we can replace all the meta-variables and holes that are left with their associated code in the original file to produce a self-contained file.
...
\paragraph{Deletion tree intersection} If some code is deleted in one of the commits, it must also be deleted in the merged result colored with at least as many tints. Moreover, if two commits delete the same code, the result pushes meta-variables as far as possible down the tree: $(\change{\alpha}{x}) \merge (\change{S(\beta)}{y}) = \change{S(\beta)}{z}$.
...
\paragraph{Preserve introduced nodes} For all nodes that are introduced in one of the commits, we know that it will be found at least once in the merged result colored with at least as many tints. However, we cannot say anything about location of these introduced nodes because their code might have been displaced by meta-variable inlining.
