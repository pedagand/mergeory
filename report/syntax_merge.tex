...
Before starting we want to define a unified syntax tree type that can represent
any parsed file in any language. For that we will use a tree with labelled
...
parent node:
\begin{typgrammar}
  \typarg{Tree}{T} &::= \synnode{\mathit{node}}{\vec{T}} \typsep
\syntoken{\mathit{token}}\\
  \typarg{Subtree}{T} &::= \synfield{\mathit{field}}{T}\\
  \typ{SyntaxTree} &::= \typarg{Tree}{\typarg{Subtree}{\typ{SyntaxTree}}}\\
\end{typgrammar}
...
Basically, a difference can be viewed as two syntax trees, a deleted one, and
an
inserted one. Therefore the simplest valid difference from code $d$ to code $i$
...
\paragraph{Grammar of difference tree}
At the end, our difference algorithm produces a syntax tree that respects the
following grammar:
\begin{typgrammar}
  \typ{SpineTree} &::= \typarg{Tree}{\typ{SpineChild}} \typsep \id \typsep
\change{\typ{ChangeTree}}{\typ{ChangeTree}}\\
  \typ{SpineChild} &::= \typarg{Subtree}{\typ{SpineTree}} \typsep
\del{\typarg{Subtree}{\typ{ChangeTree}}} \typsep
\ins{\typarg{Subtree}{\typ{ChangeTree}}}\\
